...
<script lang="ts">
  ...
  // Enum filter helpers — same pattern as TanStack toolbar
  const enumColumns = [
  ...
  // Column visibility helpers using Tzezar API
  function getHideableColumns() {
    return datagrid.columns.getLeafColumns().filter((col) => col.options.hideable);
  }

  function toggleColumnVisibility(columnId: string) {
    datagrid.handlers.column.toggleColumnVisibility(columnId);
  }
</script>
    ...
    <div class="relative ml-auto">
      ...
      {#if openDropdown === 'columns'}
        ...
        <div
          ...
          onclick={(e) => e.stopPropagation()}
        >
          {#each getHideableColumns() as column}
            <label class="flex cursor-pointer items-center gap-2 px-3 py-1.5 text-sm hover:bg-gray-50">
              <input
                type="checkbox"
